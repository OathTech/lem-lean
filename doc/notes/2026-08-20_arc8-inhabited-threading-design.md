...
is over THE call graph, which includes lem-library defs
(`Maybe_extra.fromJust`, `List_extra.head/foldl1/...`,
`Assert_extra.fail`, `Map_extra.find`) — regenerated by `make
lean-libs` with their own threaded binders, but in a SEPARATE lem
