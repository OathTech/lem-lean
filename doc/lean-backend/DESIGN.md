...
proof a hand-written module supplies (a missing or mistyped theorem fails
the build; a `sorry` is caught by the suite's token gate over the proofs
modules and by the consumer's sorry/axiom gates). The obligation states
fuel-STABILITY above the measure (the wrapper's value is fuel-independent),
not that the exhaustion arm is unreachable. The
measure may be the BACKEND-DERIVED structural size of a parameter's type
(`lemSize x` ↦ `t.lemSize x`): for every recursive block of generated
inductive types in a user module the backend emits, right after the block,
...
recursion argument bound by a trailing `function` (the head binds fewer
parameters than its type has arrows) is made a parameter by the Lean
emission alone — the `function` scrutinee is hoisted into the head as
`lemTail`, a user `fun k ->` above it under its own name — for measured
and structural definitions only, so the `.lem` keeps its shape and the
OCaml text never moves (the declare table's "point-free `function`
tails" row).
...
| (emission rule, no declare) point-free `function` tails | for a `fuel_measure`d or `structural` definition whose clause body is a trailing `function` (after lem's pattern compilation `fun x -> match x with …`), possibly under a user `fun k ->` and under the single-arm match of a destructuring parameter, the Lean emission hoists the binders into the head — the scrutinee as `lemTail` (deterministic, nameable from the `.lem` measure), user binders under their names — so the measure renderer and the structural analysis see a named parameter; the fuel sentinel is applied to the hoisted binders (`((payload) lemTail)`). Hygiene, fail-closed: refused if `lemTail` is a parameter, a body binder, a free variable or a referenced constant's name, or if a hoisted user binder would shadow a parameter / be captured by the destructuring pattern; refused on a supply-lifted def. The `.lem` and every non-Lean emitter are untouched ([USER 2026-09-04] "we don't change the lem structure for ocaml"; `2026-09-05_tails-and-pmap-laws-record.md`) |
