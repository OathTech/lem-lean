...
trailing variable-binder lambda is hoisted into the head, a `function`
scrutinee as `lemTail`, a user binder under its own name — for measured
...
- **(c) structural recursion on the data, or on a DATA measure** —
  ...
  that IS a numeral, or mentions no parameter, is refused. A measure may
  be sufficient only under a HYPOTHESIS on the arguments (`… assuming
  `H``, [USER 2026-09-05]): the wrapper is still the fuel-free form (c),
  the obligation is `H → μ ≤ fuel → worker = wrapper`, and on inputs
