...
| T8 | N | `opam:22-23` | `license: ["BSD-3-Clause" "BSD-2-Clause" "LGPL-2.0-only" "LGPL-2.1-or-later"]` is accurate as base licences (pset/pmap headers name the Library GPL v2 without "or later"; the wrapper is BSD-2). SPDX can express the exception as `LGPL-2.0-only WITH OCaml-LGPL-linking-exception` once T3 is settled. | Optional, after T3. |
