...
- **Supply-lifted top-level value bindings** become functions of the supply: each use re-runs the right-hand side with the caller's supply (state-passing semantics), not once at initialisation.
...
- **`int32`/`int64`** are `Int` newtypes without overflow semantics.
...
- `BEq` and `Ord` derived structurally with OCaml polymorphic-compare parity
